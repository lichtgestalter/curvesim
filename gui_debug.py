...
class PointViewer(tk.Tk):
    ...
    def draw_axes(self):
        self.canvas.create_line(300, 300, 600, 300, arrow=tk.LAST)  # x-axis
        self.canvas.create_line(300, 300, 300, 0, arrow=tk.LAST)    # y-axis
        self.canvas.create_line(300, 300, 300, 600, arrow=tk.LAST)  # z-axis

    def draw_points(self):
        self.canvas.delete("points")
        for point in self.points:
            label, x, y, z = point.split(",")
            x = float(x) - self.viewer_position[0]
            y = float(y) - self.viewer_position[1]
            z = float(z) - self.viewer_position[2]
            if z == 0:
                continue  # Skip the point if the z value is zero to avoid division by zero
            distance = math.sqrt(x**2 + y**2 + z**2)
            scaling_factor = 1000 / (1000 + z+0.01)
            x_2d = 300 + scaling_factor * x
            y_2d = 300 + scaling_factor * y
            self.canvas.create_text(x_2d, y_2d, text=label, tags="points")
